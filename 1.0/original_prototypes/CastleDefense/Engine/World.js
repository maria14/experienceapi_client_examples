...
World.prototype.update = function () {
    if (this.state == WorldState.RUNNING) {
        for (var i = this.runners.length - 1; i >= 0; i--) {
		    ...
		    tc_sendStatment_EndLevel(profile.scoreLevel,profile.level);
                    levelupCallback();
                }
                // delete the runner
                this.runners[i].erase(this.canvasContext);
	...
	if (this.arrows[i].pos.x==696)tc_sendStatment_Failed("arrow");
		   ...
		   tc_sendStatment_Correct("arrow");		
                    // play the sound
                    AUDIO_MANAGER.play('skeletonDeath');

                    // score the kill bonus
					profile.scoreLevel +=SCORE_KILL_BONUS;
